...
public class PluginPetalsSlInstaller extends PluginPetalsAbstractInstaller implements PluginInterface {
    ...
    public void deploy(final Instance instance) throws PluginException {
        // Deploy the SL
        this.logger.fine(this.agentId + ": deploying the SL for instance " + instance);
        final File instanceDirectory = InstanceHelpers.findInstanceDirectoryOnAgent(instance);
        final File slFile = new File(instanceDirectory, instance.getName() + ".zip");
        ...
        Thread.currentThread().setContextClassLoader(PluginPetalsSlInstaller.class.getClassLoader());
        try {
            this.connectToContainer(this.retrieveContainerInstance(instance));
            try {
                this.artifactAdministration.deployAndStartArtifact(slFile.toURI().toURL(), true);
    ...
    @Override
    protected String getManagedArtifactName(final Instance slInstance) {
        return slInstance.getName();
    }
    ...
    protected final Instance retrieveContainerInstance(final Instance slInstance)
        ...
        if (jbiComponentAbstractContainer == null
            ...
            throw new PluginException(
                    String.format(
                    "Unexpected parent for the shared library. It MUST be inherited from '%s' (current '%s')",
